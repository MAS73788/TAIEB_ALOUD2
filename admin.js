...
import {
  ...
  query, orderBy, writeBatch, serverTimestamp, onSnapshot, limit, startAfter
...
function setupTabs() {
  ...
  tabBtns.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      ['products', 'collections', 'orders', 'topProducts', 'slider', 'settings'].forEach(t => document.getElementById('tab-' + t).classList.add('hidden'));
    ...
    return `
      <div class="row bg-white border border-gray-200 rounded-xl p-3 flex items-center gap-3 flex-wrap" draggable="true" data-docid="${p._docId}">
        ...
        <div class="flex-1 min-w-[140px]">
          ...
          <p class="text-xs text-gray-400">${escapeHTML(p.catAr || '')}</p>
        </div>
    ...
    return `
      <div class="row bg-white border border-gray-200 rounded-xl p-3 flex items-center gap-3 flex-wrap" draggable="true" data-docid="${c._docId}">
        ...
        <div class="flex-1 min-w-[140px]">
          ...
          <p class="text-xs text-gray-400 truncate max-w-xs">${escapeHTML(c.descAr || '')}</p>
        </div>
